...
//            die(1111);
            ...
            if (model('Order')->addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent)) {
                ...
                $result ['message'] = ECTouch_gpl::err()->last_message();
                $result ['error'] = ECTouch_gpl::err()->error_no;
...
//        $this->assign("cart_num",$result ['cart_number']);
        die((json_encode($result['cart_number'])));
    }
    ...
    public function checkout()
    {
        ...
        $is_in_shipping_area = $this->is_in_shipping_area();
        $this->assign('is_in_shipping_area', $is_in_shipping_area);
//        var_dump($is_in_shipping_area);
        /* 对商品信息赋值 */
        $cart_goods = model('Order')->cart_goods($flow_type); // 取得商品列表，计算合计
        // var_dump($cart_goods);die;
        $this->assign('goods_list', $cart_goods);
        $shipping_type = model('Order')->shipping_type($cart_goods);
        /* 对是否允许修改购物车赋值 */
        ...
        $shipping_list = model('Shipping')->available_shipping_list2($region);
        // var_dump($shipping_list);die;
        ...
        foreach ($shipping_list as $key => $val) {
            if ($shipping_type == $val['shipping_name']) {
                $shipping_list [$key] ['current_shipping'] = 1;
                $shipping_use = 1;
            }
            //die(var_dump($val));
            // 兼容过滤ecjia配送方式
        ...
        $this->assign('shipping_use', $shipping_use);
        ...
        if ((!isset($use_bonus) || C('use_bonus') == '1') && ( $flow_type != CART_EXCHANGE_GOODS)) {
            // 取得用户可用红包
        ...
        if ($_SERVER ['REQUEST_METHOD'] == 'GET') {
            ...
            if ($act == 'signin') {
                ...
                if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION ['login_fail'] > 2)) && gd_version() > 0) {
                    ...
                    if ($_SESSION ['ectouch_gpl_verify'] !== $_POST ['captcha']) {
                        show_message(L('invalid_captcha'));
                    }
                }
    ...
    public function select_shipping()
    {
        ...
        if (empty($cart_goods) || !model('Order')->check_consignee_info($consignee, $flow_type)) {
            ...
            $result ['content'] = ECTouch_gpl::$view->fetch('library/order_total.lbi');
        }
    ...
    {
        if($_SESSION['submitTime'] && time()-$_SESSION['submitTime']<10){
            show_message("系统繁忙,请等候10秒后再次提交", '', '', 'warning');
        }else{
        ...
        $check_shipping = I('post.shipping');
        if(!$check_shipping){
            show_message("没有选择配送方式", '', '', 'warning');
        }
        ...
        $shipping_time = I('post.shipping_time', '');
        if(!empty($shipping_time)){
        $postscript.=$shipping_time;//将配送时间写到附言字段里存到数据库订单表中
    }
        $oos = L('oos.' . $how_oos);
        // 订单信息
        $order = array(
        ...
        if (C('send_service_email') && C('service_email') != '') {
            ...
            $content = ECTouch_gpl::$view->fetch('str:' . $tpl ['template_content']);
    ...
    public function change_needinv()
    {
        ...
        if (empty($cart_goods) || !model('Order')->check_consignee_info($consignee, $flow_type)) {
            ...
            die(ECTouch_gpl::$view->fetch('library/order_total.lbi'));
        }
    }
    ...
    /*
     * 判断是否在显示配送时间的配送地区*/
    public function is_in_shipping_area()
    {
        $province = $_SESSION ['flow_consignee']['province'];
        $city = $_SESSION ['flow_consignee']['city'];

        $sqlprovince = $this->model->table('region')->field('region_name')->where("region_id=" . $province)->getOne();
//        var_dump($resdata2);

        $sqlcity = $this->model->table('region')->field('region_name')->where("region_id=" . $city)->getOne();
//        var_dump($resdata);

        // 获取选择配送时间的地区
        $shipping_area = $this->model->table('shop_config')->field('value')->where("code='shipping_area'")->getOne();

        $shipping_area2 = explode("\n", $shipping_area);

//        var_dump($shipping_area2);
        foreach ($shipping_area2 as $v) {

            $v = explode("|", $v);
            $v = $this->trimall($v);
//            var_dump($v);
            if (count($v) > 1) {
                if ($sqlprovince == $v[0] && $sqlcity == $v[1]) {
