...
class UserController extends CommonController {
    ...
    public function order_detail() {
        ...
        if ($order === false) {
            ECTouch_gpl::err()->show(L('back_home_lnk'), './');
            exit();
        }
...
//die(htmlentities(var_dump($order['pay_online'])));
        // 订单 支付 配送 状态语言项
    ...
    public function affirm_received() {
        ...
        if (model('Users')->affirm_received($order_id, $this->user_id)) {
            ...
            ECTouch_gpl::err()->show(L('order_list_lnk'), url('order_list'));
        }
    }
    ...
    public function edit_surplus() {
        ...
        if ($surplus <= 0) {
            ECTouch_gpl::err()->add(L('error_surplus_invalid'));
            ECTouch_gpl::err()->show(L('order_detail'), url('order_detail', array(
                'order_id' => $order_id
            )));
        }
        ...
        if ($order['pay_status'] != PS_UNPAYED || $order['order_amount'] <= 0) {
            ECTouch_gpl::err()->add(L('error_order_is_paid'));
            ECTouch_gpl::err()->show(L('order_detail'), url('order_detail', array(
                'order_id' => $order_id
            )));
        }
        ...
        if ($surplus > $user['user_money'] + $user['credit_line']) {
            ECTouch_gpl::err()->add(L('error_surplus_not_enough'));
            ECTouch_gpl::err()->show(L('order_detail'), url('order_detail', array(
                'order_id' => $order_id
            )));
        }
    ...
    public function bonus() {
        if (IS_POST) {
            ...
            if (model('Users')->add_bonus($this->user_id, $bonus_sn)) {
                ...
                ECTouch_gpl::err()->show(L('back_up_page'), url('bonus'));
            }
        }
        // 分页
        $filter['page'] = '{page}';
    ...
    public function add_booking() {
        if (IS_POST) {
            ...
            if (model('ClipsBase')->add_booking($booking)) {
                ...
                ECTouch_gpl::err()->show(L('booking_list_lnk'), url('booking_list'));
            }
        }
        $goods_id = I('get.id', 0);
        if ($goods_id == 0) {
    ...
    public function login() {
        ...
        if (IS_POST) {
            ...
            if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2))) {
                ...
                // 检查验证码
                if ($_SESSION['ectouch_gpl_verify'] !== strtoupper($_POST['captcha'])) {
                    show_message(L('invalid_captcha'), L('relogin_lnk'), url('login', array(
                        'referer' => urlencode($this->back_act)
                            )), 'error');
                }
            ...
            if (self::$user->login($username, $password, isset($_POST['remember']))) {
               ...
               // $this->message("登录成功",$jump_url);
               // show_message("登录成功","返回首页",url('index'));
                //show_message("登录成功", L('back_home_lnk') ,  url('index/index'), 'info');
               // $this->redirect($jump_url);

                $jump_url = empty($this->back_act) ? url('index/index') : $this->back_act;
                show_message("登录成功", L('back_home_lnk') ,  $jump_url, 'info');
            } else {
                $_SESSION['login_fail']++;
    ...
    public function register() {
        ...
        if (IS_POST) {
            ...
            elseif (1 == $enabled_sms) {
                ...
                ECTouch_gpl::err()->show(L('sign_up'), url('register'));
            }
