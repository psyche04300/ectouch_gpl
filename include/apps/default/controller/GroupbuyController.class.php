...
class GroupbuyController extends CommonController
{
    ...
    public function info()
    {
        ...
        // 会员等级价格
        $this->assign('rank_prices', model('GroupBuyBase')->get_user_rank_prices($group_buy_id, $group_buy['cur_price']));
        // die(var_dump($_SESSION['user_rank']));
    ...
    public function buy()
    {
        ...
        /* 查询：判断用户限购数量 */
        if ($group_buy['user_restrict_amount'] > 0 && $number > $group_buy['user_restrict_amount']) {
            show_message('超出用户限购数量', '', '', 'error');
        }
        /* 查询：取得规格 */
        $specs = '';
        ...
        $sql = "SELECT rank.rank_id,mp.user_price, " .
            "IFNULL(mp.user_price, " . $goods_price . ") AS goods_price " .
            "  FROM  " . $this->model->pre . "user_rank  AS rank  LEFT JOIN " . $this->model->pre . "group_buy_member_price  AS mp ON  mp.user_rank=rank.rank_id WHERE rank.rank_id=" . $_SESSION['user_rank'] . " AND mp.act_id=" . $group_buy_id;
        $row2 = $this->model->query($sql);
        $goods_price = $row2[0]['goods_price'];
        $cart = array(
