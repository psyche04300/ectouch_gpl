...
class CommonController extends BaseController
{
    ...
    protected function ecshop_init()
    {
        ...
        if (preg_match('/micromessenger/', $ua)) {
            ...
            if (!isset($_SESSION["openid"]) || empty($_SESSION["openid"])) {//openid为空
                ...
                if (!$_SESSION['user_id'] || !$_SESSION['user_name'] ) {
                    ...
                    if ($_SESSION['openid'] && !$_COOKIE['ECS']['user_logout']) {
                        $where=array();
        ...
        if (!empty($stylename)) {
            $this->assign('ectouch_gpl_css_path', __ROOT__ . '/themes/' . C('template') . '/css/ectouch_gpl_' . C('stylename') . '.css');
        } else {
            $this->assign('ectouch_gpl_css_path', __ROOT__ . '/themes/' . C('template') . '/css/ectouch_gpl.css');
        }
    }
}

class_alias('CommonController', 'ECTouch_gpl');
