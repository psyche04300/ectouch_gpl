...
class IndexModel extends CommonModel {
    ...
    public function goods_list_for_id($id, $limit = 10, $start = 0) {

       $where='g.cat_id='.intval($id);
        ...
        $sql .= ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND ' . $where;
...
//                $goods[$key]['mysc'] = $rs;
            ...
            $type_goods[] = $goods[$key];
        }
        return $type_goods;
    }
    function getCat($cat_id){

        $cat=$this->row("SELECT cat_id, cat_name FROM " . $this->pre . "category WHERE cat_id=".$cat_id." AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC");
        return $cat;
