...
class OrderModel extends BaseModel
{
    ...
    {
        $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number ,market_price, goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, is_shipping, " .
        ...
        foreach ($arr as $key => $value) {
            $goods = model('Goods')->get_goods_info($value['goods_id']);
            $arr[$key]['formated_market_price'] = price_format($value['market_price'], false);
            $arr[$key]['formated_goods_price'] = price_format($value['goods_price'], false);
            $arr[$key]['formated_subtotal'] = price_format($value['subtotal'], false);
            $arr[$key]['shipping_type'] = $goods['shipping_type'];
    ...
    function shipping_type($cart_goods)
    {
        $shipping_type = '';
        foreach ($cart_goods as $goods) {
            $shipping_type[] = $goods['shipping_type'];
        }
        // var_dump($shipping_type);die;
        if (in_array('组合装产品配送', $shipping_type)) {
            return '组合装产品配送';
        }
        if (in_array('冷冻品配送', $shipping_type)) {
            return '冷冻品配送';
        }
        if (in_array('冷藏品配送', $shipping_type)) {
            return '冷藏品配送';
        }
        if (in_array('常温品配送', $shipping_type)) {
            return '常温品配送';
        }
        return '';
    }
    ...
    function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
    {
        ...
        if (empty($goods)) {
            ECTouch_gpl::err()->add(L('goods_not_exists'), ERR_NOT_EXISTS);
        ...
        if ($parent > 0) {
            ...
            $res = $this->row($sql);
            if ($res['count'] == 0) {
                ECTouch_gpl::err()->add(L('no_basic_goods'), ERR_NO_BASIC_GOODS);
        ...
        if ($goods['is_on_sale'] == 0) {
            ECTouch_gpl::err()->add(L('not_on_sale'), ERR_NOT_ON_SALE);
        ...
        if (empty($parent) && $goods['is_alone_sale'] == 0) {
            ECTouch_gpl::err()->add(L('cannt_alone_sale'), ERR_CANNT_ALONE_SALE);
        ...
        if (C('use_storage') == 1) {
            ...
            if ($num > $goods['goods_number']) {
                ECTouch_gpl::err()->add(sprintf(L('shortage'), $goods['goods_number']), ERR_OUT_OF_STOCK);
            ...
            if (model('GoodsBase')->is_spec($spec) && !empty($prod)) {
                if (!empty($spec)) {
                    ...
                    if ($num > $product_info['product_number']) {
                        ECTouch_gpl::err()->add(sprintf(L('shortage'), $product_info['product_number']), ERR_OUT_OF_STOCK);
        ...
        if ($num > 0) {
            ...
            if ($row) { //如果购物车已经有此物品，则更新
                ...
                if (C('use_storage') == 0 || $num <= $goods_storage) {
                    ...
                    ECTouch_gpl::err()->add(sprintf(L('shortage'), $num), ERR_OUT_OF_STOCK);
    ...
    function send_order_bonus($order_id)
    {
        ...
        if ($bonus_list) {
            ...
            if ($count > 0) {
                ...
                ECTouch_gpl::view()->assign('user_name', $user['user_name']);
                ECTouch_gpl::view()->assign('count', $count);
                ECTouch_gpl::view()->assign('money', $money);
                ECTouch_gpl::view()->assign('shop_name', C('shop_name'));
                ECTouch_gpl::view()->assign('send_date', local_date(C('date_format')));
                ECTouch_gpl::view()->assign('sent_date', local_date(C('date_format')));
    ...
    function add_package_to_cart($package_id, $num = 1)
    {
        ...
        if (C('use_storage') == '1' && model('Order')->judge_package_stock($package_id)) {
            ECTouch_gpl::err()->add(sprintf(L('shortage'), 1), ERR_OUT_OF_STOCK);
