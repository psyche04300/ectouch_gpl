...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1, $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr = '', $url_format = '', $sch_array = '') {
    ...
    ECTouch_gpl::view()->assign('pager', $pager);
}
    ...
    if (!empty($upload['tmp_name'])) {
        ...
        if (!empty($ftype)) {
            ...
            if (!move_upload_file($upload['tmp_name'], $target)) {
                ECTouch_gpl::err()->add(L('upload_file_error'), 1);
            ...
            ECTouch_gpl::err()->add(L('upload_file_type'), 1);

            return false;
        }
    } else {
        ECTouch_gpl::err()->add(L('upload_file_error'));
        return false;
    }
}
...
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true) {
    ...
    $msg['type'] = $type;
    if (is_null(ECTouch_gpl::view()->get_template_vars('helps'))) {
        ECTouch_gpl::view()->assign('helps', model('Article')->get_shop_help()); // 网店帮助
    }

    ECTouch_gpl::view()->assign('title', L('tips_message'));
    ECTouch_gpl::view()->assign('auto_redirect', $auto_redirect);
    ECTouch_gpl::view()->assign('message', $msg);
    ECTouch_gpl::view()->display('message.dwt');
...
function assign_template($ctype = '', $catlist = array()) {
    ECTouch_gpl::view()->assign('image_width', C('image_width'));
    ECTouch_gpl::view()->assign('image_height', C('image_height'));
    ECTouch_gpl::view()->assign('points_name', C('integral_name'));
    ECTouch_gpl::view()->assign('qq', explode(',', C('qq')));
    ECTouch_gpl::view()->assign('ww', explode(',', C('ww')));
    ECTouch_gpl::view()->assign('ym', explode(',', C('ym')));
    ECTouch_gpl::view()->assign('msn', explode(',', C('msn')));
    ECTouch_gpl::view()->assign('skype', explode(',', C('skype')));
    ECTouch_gpl::view()->assign('stats_code', C('stats_code'));
    ECTouch_gpl::view()->assign('shop_notice', C('shop_notice'));
    ECTouch_gpl::view()->assign('user_notice', C('user_notice'));
    ECTouch_gpl::view()->assign('copyright', sprintf(L('copyright'), date('Y'), C('shop_name')));
    ECTouch_gpl::view()->assign('shop_name', C('shop_name'));
    ECTouch_gpl::view()->assign('service_email', C('service_email'));
    ECTouch_gpl::view()->assign('service_phone', C('service_phone'));
    ECTouch_gpl::view()->assign('shop_address', C('shop_address'));
    ECTouch_gpl::view()->assign('licensed', license_info());
    ECTouch_gpl::view()->assign('ecs_version', VERSION);
    ECTouch_gpl::view()->assign('icp_number', C('icp_number'));
    ECTouch_gpl::view()->assign('username', !empty($_SESSION['user_name']) ? $_SESSION['user_name'] : '');
    ECTouch_gpl::view()->assign('category_list', cat_list(0, 0, true, 2, false));
    ECTouch_gpl::view()->assign('catalog_list', cat_list(0, 0, false, 1, false));
    ECTouch_gpl::view()->assign('navigator_list', model('Common')->get_navigator($ctype, $catlist));  //自定义导航栏
    ...
    ECTouch_gpl::view()->assign('searchkeywords', $searchkeywords);
}
...
function license_info() {
    if (C('licensed') > 0) {
        $license = '<a href="http://www.ecshop.com/license.php?product=ectouch_gpl_free&url=' . urlencode(__URL__) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    } else {
        return '';
...
function get_order_sn() {
    /* 选择一个随机的方案 */
    return date('Ymdhis') . str_pad(mt_rand(1, 9999999), 7, '0', STR_PAD_LEFT).$_SESSION['user_id'];
}
...
function edit_password($user_id, $old_password, $new_password = '', $code = '') {
    ...
    if (ECTouch_gpl::user()->edit_password($user_id, $old_password, $new_password, $code)) {
        return true;
    } else {
        ECTouch_gpl::err()->add(L('edit_password_failure'));
...
function check_userinfo($user_name, $email) {
    ...
    /* 检测用户名和邮件地址是否匹配 */
    $user_info = ECTouch_gpl::user()->check_pwd_info($user_name, $email);
    if (!empty($user_info)) {
        return $user_info;
    } else {
        return false;
...
function send_pwd_email($uid, $user_name, $email, $code) {
    ...
    ECTouch_gpl::view()->assign('user_name', $user_name);
    ECTouch_gpl::view()->assign('reset_email', $reset_email);
    ECTouch_gpl::view()->assign('shop_name', C('shop_name'));
    ECTouch_gpl::view()->assign('send_date', date('Y-m-d'));
    ECTouch_gpl::view()->assign('sent_date', date('Y-m-d'));
