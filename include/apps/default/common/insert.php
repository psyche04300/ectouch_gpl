...
function insert_ads($arr) {
    ...
    $need_cache = ECTouch_gpl::view()->caching;
    ECTouch_gpl::view()->caching = false;

    ECTouch_gpl::view()->assign('ads', $ads);
    $val = ECTouch_gpl::view()->fetch($position_style);

    ECTouch_gpl::view()->caching = $need_cache;
...
function insert_member_info() {
    ...
    if ($_SESSION['user_id'] > 0) {
        ECTouch_gpl::view()->assign('user_info', model('Users')->get_user_info());
    } else {
        if (!empty($_COOKIE['ECS']['username'])) {
            ECTouch_gpl::view()->assign('ecs_username', stripslashes($_COOKIE['ECS']['username']));
        }
        $captcha = intval(C('captcha'));
        if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0) {
            ECTouch_gpl::view()->assign('enabled_captcha', 1);
            ECTouch_gpl::view()->assign('rand', mt_rand());
        }
    }
    $output = ECTouch_gpl::view()->fetch('library/member_info.lbi');
...
function insert_comments($arr) {
    $need_cache = ECTouch_gpl::view()->caching;
    $need_compile = ECTouch_gpl::view()->force_compile;

    ECTouch_gpl::view()->caching = false;
    ECTouch_gpl::view()->force_compile = true;
    ...
    ECTouch_gpl::view()->assign('username', stripslashes($_SESSION['user_name']));
    ECTouch_gpl::view()->assign('email', $_SESSION['email']);
    ECTouch_gpl::view()->assign('comment_type', $arr['type']);
    ECTouch_gpl::view()->assign('id', $arr['id']);
    $comments = model('Comment')->get_comment_info($arr['id'], 0);
    ECTouch_gpl::view()->assign('comments_info', $comments);
    //全部评论
    $cmt = model('Comment')->assign_comment($arr['id'], $arr['type']);
    ECTouch_gpl::view()->assign('comment_list', $cmt['comments']);
    ECTouch_gpl::view()->assign('pager', $cmt['pager']);
    //好评
    $cmt_fav = model('Comment')->assign_comment($arr['id'], $arr['type'], '1');
    ECTouch_gpl::view()->assign('comment_fav', $cmt_fav['comments']);
    ECTouch_gpl::view()->assign('pager_fav', $cmt_fav['pager']);
    //中评
    $cmt_med = model('Comment')->assign_comment($arr['id'], $arr['type'], '2');
    ECTouch_gpl::view()->assign('comment_med', $cmt_med['comments']);
    ECTouch_gpl::view()->assign('pager_med', $cmt_med['pager']);
    //差评
    $cmt_bad = model('Comment')->assign_comment($arr['id'], $arr['type'], '3');
    ECTouch_gpl::view()->assign('comment_bad', $cmt_bad['comments']);
    ECTouch_gpl::view()->assign('pager_bad', $cmt_bad['pager']);
    $val = ECTouch_gpl::view()->fetch('library/comments_list.lbi');

    ECTouch_gpl::view()->caching = $need_cache;
    ECTouch_gpl::view()->force_compile = $need_compile;
...
function insert_bought_notes($arr) {
    ...
    ECTouch_gpl::view()->assign('notes', $bought_notes);
    ECTouch_gpl::view()->assign('pager', $pager);


    $val = ECTouch_gpl::view()->fetch('library/bought_notes.lbi');
...
function insert_vote() {
    $vote = get_vote();
    if (!empty($vote)) {
        ECTouch_gpl::view()->assign('vote_id', $vote['id']);
        ECTouch_gpl::view()->assign('vote', $vote['content']);
    }
    $val = ECTouch_gpl::view()->fetch('library/vote.lbi');
