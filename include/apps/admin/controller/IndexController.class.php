...
class IndexController extends AdminController
{
    ...
    public function welcome()
    {
        ...
        // 检测是否授权
        $data = array('appid' => ECTOUCH_GPL_AUTH_KEY);
    ...
    public function login()
    {
        if (IS_POST) {
            ...
            if (($captcha_admin & CAPTCHA_ADMIN) && gd_version() > 0)
            {
                ...
                $msg = Check::rule(array(
                    array(
                        Check::same($captcha, $_SESSION['ectouch_gpl_verify']),
                        L('captcha_error')
                    )
                ));
            }
            ...
            if (! empty($userInfo)) {
                ...
                if (! empty($remember)) {
                    ...
                    setcookie('ECTOUCH_GPLCP[ADMIN_ID]', $userInfo['user_id'], $time);
                    setcookie('ECTOUCH_GPLCP[ADMIN_PWD]', md5(md5($userInfo['user_id'] . $userInfo['user_name']) . C('hash_code')), $time);
                }
                $result = array(
                    'err' => 0,
                    'msg' => 'login success'
    ...
    public function license()
    {
        if (IS_POST) {
            ...
            $data = array('license'=>$license, 'appid' => ECTOUCH_GPL_AUTH_KEY);
