...
class UpgradeController extends AdminController
{
    ...
    // md5验证地址
    private $_upgrademd5 = 'http://www.ectouch_gpl.cn/upgrademd5/';
    // 补丁地址
    private $_patchurl = 'http://download.ectouch_gpl.cn/upgrade/1.0/patch/';
    ...
    public function init()
    {
        ...
        foreach ($pathlist as $k => $v) {
            ...
            if (file_exists($configpath)) {
                ...
                // 版本文件地址
                $content = "<?php\ndefine('APPNAME', '".$config['APPNAME']."');\ndefine('VERSION', '".$config['VERSION']."');\ndefine('RELEASE', '".$release."');\ndefine('ECTOUCH_GPL_AUTH_KEY', '".ECTOUCH_GPL_AUTH_KEY."');";
    ...
    public function checkfile()
    {
        ...
        if (! empty($do)) {
            $this->ec_readdir('.');
            // 读取接口
            $ectouch_gpl_md5 = Http::doGet($this->_upgrademd5 . RELEASE . '_' . $this->patch_charset . ".php");
            $ectouch_gpl_md5_arr = json_decode($ectouch_gpl_md5, 1);
            $ectouch_gpl_md5_arr = empty($ectouch_gpl_md5_arr) ? array():$ectouch_gpl_md5_arr;
            // 计算数组差集
            $diff = array_diff($ectouch_gpl_md5_arr, $this->md5_arr);
            // 丢失文件列表
            $lostfile = array();
            foreach ($ectouch_gpl_md5_arr as $k => $v) {
            ...
            // 未知文件列表
            $unknowfile = array_diff(array_keys($this->md5_arr), array_keys($ectouch_gpl_md5_arr));
            // 赋值
