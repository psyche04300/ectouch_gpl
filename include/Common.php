 ...
 * @return string
 */
function xml_encode($data, $root = 'ectouch_gpl', $item = 'item', $attr = '', $id = 'id', $encoding = 'utf-8') {
    if (is_array($attr)) {
        $_attr = array();
...
function cookie($name = '', $value = '', $option = null) {
    ...
    if ('' === $value) {
        ...
        if (is_null($value)) {
            ...
            // 设置cookie
            if (is_array($value)) {
                $value = 'ectouch_gpl:' . json_encode(array_map('urlencode', $value));
            }
 ...
 * @param string $appid
 */
function site_info($appid = ECTOUCH_GPL_AUTH_KEY){
