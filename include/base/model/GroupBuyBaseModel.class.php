...
/**
     ...
     * @return array
     */
    function get_user_rank_prices($act_id, $shop_price)
    {
        $sql = "SELECT rank_id, user_rank,IFNULL(mp.user_price, r.discount * $shop_price / 100) AS price, r.rank_name, r.discount " . 'FROM ' . $this->pre . 'user_rank AS r ' . 'LEFT JOIN ' . $this->pre . "group_buy_member_price AS mp " . "ON mp.act_id = '$act_id' AND mp.user_rank = r.rank_id " . "WHERE r.show_price = 1 OR r.rank_id = '$_SESSION[user_rank]'";
        $res = $this->query($sql);
        $arr = array();
        foreach ($res as $row) {
    ...
    function group_buy_info($group_buy_id, $current_num = 0) {
        ...
        $group_buy['sales_count'] = $group_buy['sales_count'] ? $group_buy['sales_count'] : 999;
