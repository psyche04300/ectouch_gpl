...
$shipping_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/shipping/changwenpinpeisong.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 配送方式插件的代码必须和文件名保持一致 */
    $modules[$i]['code'] = 'changwenpinpeisong';

    $modules[$i]['version'] = '1.0.0';

    /* 配送方式的描述 */
    $modules[$i]['desc'] = 'changwenpinpeisong_desc';
    ...
    /* 插件的作者 */
    $modules[$i]['author'] = '青竹林';

    /* 插件作者的官方网站 */
    $modules[$i]['website'] = 'http://www.qzl188.com';
    ...
    $modules[$i]['configure'] = array(
        ...
        array('name' => 'min_fee', 'value' => 0),    /* 起步价 */
        array('name' => 'free_money_unit_2', 'value' => 0),
        array('name' => 'free_money_unit_3', 'value' => 0),
    );
    ...
    /* 打印单背景 */
    $modules[$i]['print_bg'] = '/images/receipt/qzl.jpg';
...
class changwenpinpeisong
{
     ...
     * @return null
     */
    function changwenpinpeisong($cfg = array())
    {
   ...
   /* function calculate($goods_weight, $goods_amount, $goods_number)
    {
        if ($this->configure['free_money'] > 0 && $goods_amount >= $this->configure['free_money']) {
            ...
            if($this->configure['min_fee'] && $fee<=$this->configure['min_fee']){
                $fee=$this->configure['min_fee'];
            }
    ...
    function calculate($goods_weight, $goods_amount, $goods_number)
    {
        ...
        if ($this->configure['fee_compute_mode'] == 'by_number') {
            ...
            if ($this->configure['free_money'] > 0 && $goods_number >= $this->configure['free_money']) {
                return 0;
            }

            $fee = $goods_number * $this->configure['item_fee'];
            if ($goods_number == 2) {
                $fee = $this->configure['free_money_unit_2'];
            }
            if ($goods_number >= 3) {
                $fee = $this->configure['free_money_unit_3'];
            }
        } else {
            if ($this->configure['free_money'] > 0 && $goods_weight >= $this->configure['free_money']) {
                return 0;
            }
            if ($goods_weight > 1) {
                $fee += (ceil(($goods_weight - 1))) * $this->configure['step_fee'];
